  ...
  methods: {
    ...
    resetInactivityTimer () {
      if (this.timerStarted && !this.idleDialogVisible) {
        ...
        console.log('[App] resetInactivityTimer: ignored (timerStarted=', this.timerStarted, ', idleDialogVisible=', this.idleDialogVisible, ')')
      }
    },

    showIdleDialog () {
      console.log('[App] showIdleDialog called')
      this.idleDialogVisible = true
      this.idleCountdown = 5
      this.idleDialogTimer = setInterval(() => {
        if (this.idleCountdown > 1) {
          this.idleCountdown--
        } else {
      ...
      try {
        swh.initStelWebEngine(that.$store, f.default, that.$refs.stelCanvas, function () {
          ...
          if (!that.dataSourceInitDone) {
            ...
            that.$store.state.stel.constellations.labels_visible = true
            core.constellations.images_visible = true
            that.$store.state.stel.constellations.images_visible = true
